...
func TestEqualsGeneralLoggerConfig(t *testing.T) {
	...
	var config LoggerConfig = GeneralLoggerConfig{Common: &commonConfig, Severity: common.ERROR_SEVERITY, IsCallerToSet: true, WithErrorCallStack: false}
...
func TestNotEqualsGeneralLoggerConfig(t *testing.T) {
	commonConfig := CommonLoggerConfig{LoggerType: LOGGER_GENERAL}
	config := GeneralLoggerConfig{Common: &commonConfig, Severity: common.ERROR_SEVERITY, IsCallerToSet: true, WithErrorCallStack: false}
	...
	checkNotEqualsGeneralLoggerConfig(&config, func(otherConfig *GeneralLoggerConfig) {
		otherConfig.WithErrorCallStack = !otherConfig.WithErrorCallStack
	}, "general WithErrorCallStack diff", t)
}
